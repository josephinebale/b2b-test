...
The current **node** can be a location or a grouping. An arm is a grouping, not a third node face: it uses the same Dashboard / Supportables / Workers sections, its children are always groupings, and its counts resolve recursively. The live hierarchy is Organisation → Arm → Grouping → Location. Operational groupings remain recursive, so Careforce area still contains two caseloads. The other scopes are the **organisation** and the signed-in person. Locations may appear beneath more than one operational grouping.
...
At any grouping node, the second header tier renders **Dashboard / Supportables / Workers**, so its page heading starts at the same height as a location page heading. Notifications remains a header utility. The same three-section face serves a region, a Lifestyles grouping, a caseload, and the Careforce area. Dashboard is the fixed oversight face: Requests waiting and Platform use. Supportables lists the node's direct children, with child groupings before direct locations; it does not flatten descendant locations into rows. Workers contains the existing read-only ranked provider population. Entering a child grouping opens that grouping's Dashboard. Entering a location lands on its Bookings schedule. A route unavailable at the active node continues to resolve to that node's default rather than erroring. Organisation settings and Your account remain available from the account menu because they are outside the supportable scope. Old `/team` profile and list links still rewrite to `/workers`, which resolves according to the active node.
...
`GROUPINGS` contains fifteen records: five arms and ten operational groupings. Arms carry no sector attribute; sector remains on personas, locations, and the operational grouping records as before.

- **Cerebral Palsy Alliance arms:** SIL → Northern Sydney; Lifestyles → Northern Lifestyles; Careforce → Careforce area.
...
Request pressure, approvals, messages, workers, and booking patterns differ by location. Grouping Supportables lists direct children: groupings first, then direct locations. A child grouping row carries its name, a count of what sits beneath it in Houses and centres / Clients language (houses, centres, clients, or a mixed line), and the summed waiting-work line; it carries no location marker, service type, or suburb. The page description uses the same kind language as the child-grouping section title: the caseloads in Careforce area, the regions in Greater Sydney, the houses and centres in Northern Sydney. It does not say children, nodes, or supportables. Selecting it enters that grouping's Dashboard. A direct location row retains its existing shape and enters its Bookings schedule with that grouping preserved in the breadcrumb. The moderator persona picker contains all twelve personas grouped under all three organisations, so the moderator can change organisation or entry point without putting an organisation-wide switcher in participant chrome.
...
The **48px** second tier (`--header-nav-height`) holds Dashboard / Supportables / Workers at a grouping and Bookings / Workers / Messages / Location settings at a location. A home-and-community client names the fourth location item **[Name] settings**. The section row keeps its 14/20 type, 3px body-text underline, existing quiet hover surface, and inline Bookings and Messages badges. Active labels reserve their 700-weight width through the hidden bold copy in `.main-nav-label`, so the row does not shift. The hover fill alone uses the product’s existing literal `150ms ease`; there is still no motion token. `navigate` continues to notify route subscribers in the same tick, and `/` resolves to Bookings for a location, so the active section is known on first render.

The breadcrumb shows organisation, arm, selected grouping path, and current location. `BREADCRUMB_MENU_MODE` in `src/components/NodeBreadcrumb.tsx` chooses what a crumb’s dropdown lists. The trial default is **children**: a crumb opens what is nested inside it, one level only, and carries no chevron only when nothing is. Organisation is the top of that chain, not an exception — it opens its arms. A location is the end of the trail, so it never opens. Organisation is still not a destination: selecting an arm enters that arm’s Dashboard. The sibling path is kept behind the same constant, where organisation has no menu because it has no siblings across the hard boundary. Flipping the constant back to `'siblings'` restores lateral menus without deleting either model.

Children-menu rows are a compact Supportables family: name at body size, a quieter detail line beneath. A grouping row uses `groupingContentsSummary` (“5 houses · 1 client”); a location row uses service type and suburb (“SIL house · Dee Why”). No marker, icon, or new tone. Menus still cap at twenty rows with a quiet **N more** line, reuse the account-menu pattern, and never contain search or fall through to the hidden header search.
...
**Breadcrumb** (`NodeBreadcrumb`): organisation, arm, the active grouping path, and current location. The live trial opens **children** of the crumb: one level of whatever is nested inside it. A crumb carries a chevron when it has children and none when it does not. Organisation opens its arms. A location never opens. The sibling model remains in the same file behind `BREADCRUMB_MENU_MODE`, where organisation has no menu because it has no siblings. The path is the one the person came down, anchored on their entry node, because a location with two parents cannot be placed from its own data. Overflow drops ancestors from the left and replaces them with an interactive ellipsis that lists the hidden path. It carries no marker because the full location name is already present. The bottom-right moderator persona menu remains the behind-the-scenes route among all twelve seeded entry points and organisations.

Both dropdown models exist so they can be compared in research. Children matches how the team think about moving down the tree; siblings answers three catalogue jobs that are all lateral. The children version overlaps with the Supportables section by design. The sibling version answers lateral movement and the children version does not, so whichever wins, lateral movement still needs an answer somewhere.
...
- The breadcrumb dropdown is on a **children trial** (`BREADCRUMB_MENU_MODE = 'children'`), with the sibling path kept for comparison. Children matches moving down the tree and overlaps with Supportables by design. Siblings answered three catalogue jobs that are all lateral. Whichever model wins, **lateral movement still needs an answer somewhere** — children does not restore it.

...
- **Check:** `npm run lint` and `node --test tests/*.test.ts`. There are **238** source-file assertion tests — update them, do not delete them.
...
Each record carries `{ id, organisation, sector, theme, job, saidBy, iaRelevant, resolvesAt }`. IDs follow workbook order (`job-001` through `job-119`). `iaRelevant` is a boolean attribute, not a separate list. `resolvesAt` is the product path where the job is answered (for example `Location > Bookings > Request`); it is an empty string for the 61 jobs the prototype does not address. The workbook’s **Validated?** and **Notes** authoring columns are deliberately not seeded or rendered.

The screen has one list with a live matching count. A row gives the participant-worded job room to read, then shows who said it plus theme, organisation, sector, and IA relevance. When `resolvesAt` has a value, the row closes with a metadata line **under the tags**: a bold **Where it resolves:** label then the path, at `text-xs text-text-secondary` like the sub-lines on the IA tree. When it is blank the row does not carry that line at all — no empty label, placeholder, or “not addressed”. There is no expandable evidence section: the job itself now carries the participant’s terms of phrase. Organisation, sector, theme, and IA relevance are four cumulative filters joined with AND; selecting more than one narrows to jobs matching every active choice. **All** leaves that dimension unconstrained. The single empty state appears when no job matches the active combination.
